...
public class ClubeDAO {
	...
	//Teste Exercicio Bety 
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true)
	public List<Clube> findAllTeste() {
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true)
	public List<Clube> findById(Integer idClube) {
		String jpql = " SELECT c from Clube c where id = :id order by c.nome";
		Query query = entityManager.createQuery(jpql);
		query.setParameter("id",idClube );
		List<Clube> clubes = (List<Clube>) query.getResultList();
		return clubes;
	}
